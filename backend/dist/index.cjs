...
var viewsPath = import_node_path.default.join(__dirname, "views");
...
var DB_ID = process.env.APPWRITE_DATABASE_ID;
var COL_PROJECTS = process.env.APPWRITE_COLLECTION_PROJECTS;
...
function sign(payload) {
  return import_jsonwebtoken.default.sign(payload, process.env.JWT_SECRET, { expiresIn: "2h" });
}
...
app.post("/api/contact-messages", async (req, res) => {
  try {
    ...
    const doc = await db.createDocument(DB_ID, COL_MESSAGES, import_node_appwrite.ID.unique(), {
      subject,
      senderEmail,
      body: message,
...
app.post("/admin/login", async (req, res) => {
  ...
  try {
    const { documents } = await db.listDocuments(DB_ID, COL_ADMINS, [import_node_appwrite.Query.equal("username", [username])]);
    const u = documents[0];
    if (u && await import_bcryptjs.default.compare(password, u.passwordHash)) {
...
app.get("/admin/messages", requireAuth, async (req, res) => {
  try {
    let documents = [];
    try {
      const list = await db.listDocuments(DB_ID, COL_MESSAGES, [import_node_appwrite.Query.orderDesc("createdAt")]);
...
app.post("/admin/projects", requireAuth, upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const doc = await db.createDocument(DB_ID, COL_PROJECTS, import_node_appwrite.ID.unique(), {
      title,
      description,
